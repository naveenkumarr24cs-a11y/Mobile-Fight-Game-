...
class GameView @JvmOverloads constructor(
    ...
    // Engine Components
    val spriteLoader = SpriteLoader(context)
    ...
    fun update(dt: Float) {
        ...
        when (gameState) {
            ...
            GameState.ACTIVE_FIGHTING -> {
                ...
                // Update inputManager held inputs for player1
                inputManager.applyHeldInputs(player1)

                // Update Fighters
                player1.update(dt, viewWidth, player2, spriteLoader)
                player2.update(dt, viewWidth, player1, spriteLoader)
    ...
    private fun startRound() {
        ...
        combatSystem.reset()
        aiController.reset()
        inputManager.reset(player1)   // ← clear joystick & button states between rounds
    ...
    fun drawGame(canvas: Canvas) {
        ...
        // Draw Fighters and Effects only if in an active match state
        val isMatchState = gameState == GameState.CINEMATIC_INTRO || 
                           gameState == GameState.ACTIVE_FIGHTING || 
                           gameState == GameState.ROUND_END || 
